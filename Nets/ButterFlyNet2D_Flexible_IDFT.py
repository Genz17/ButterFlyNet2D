...
class ButterFlyNet2D_IDFT(nn.Module):
    ...
    def forward(self, input_data):
        ...
        # ax.scatter(self.plot_x, self.plot_y, (out_A.cpu())[0][0].real.detach().numpy())
        # ax.scatter(self.plot_x, self.plot_y, (out_A.cpu())[0][0].imag.detach().numpy())
        # plt.show()
        return out_A
            ...
            for channel in range(input_data.shape[1]):
                for y in range(input_data.shape[2]):
                    for x in range(input_data.shape[3]):
                        data_split[batch, 4*channel, y, x] = \
                            input_data[batch, channel,y, x].real
                        data_split[batch, 4*channel+1, y, x] = \
                            input_data[batch, channel, y, x].imag
                        data_split[batch, 4*channel+2, y, x] = \
                            -input_data[batch, channel, y, x].real
                        data_split[batch, 4*channel+3, y, x] = \
                            -input_data[batch, channel, y, x].imag
