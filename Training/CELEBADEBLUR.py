...
Net = ButterFlyNet_DEBLUR(64,5).cuda()
...
for epoch in range(epochs):
    for step, (image, label) in enumerate(train_loader):
        ...
        print('CELEBADEBLUR: Train Epoch: {}, [{}/{} ({:.2f}%)]\tLoss: {:.6f}'.format(epoch, step * len(image),
                                                                                len(train_loader.dataset),
                                                                                100 * step / len(train_loader),
                                                                                loss.item()))

torch.save(Net.state_dict(), '64GRAYCelebadeblurring.pth')
