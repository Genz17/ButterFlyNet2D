...
def full(train_loader,batch_size,Net,mask):
...
def overlap(train_loader,batch_size,Net,mask,image_size,local_size):
    for step, (image, label) in enumerate(train_loader):
        with torch.no_grad():
            ...
            for i in range(3):
                ...
                for ii in range(lap_time ** 2):
                    ...
                    (ii % lap_time) * local_size:((ii % lap_time) + 1) * local_size] = done[ii * batch_size:(
                                                                                                                        ii + 1) * batch_size,
                                                                                       :, :, :]
            before = sum([-10 * np.log10((torch.norm(maskedimage[i] - image[i], 'fro').item())
